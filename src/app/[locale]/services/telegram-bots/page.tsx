...
export async function generateMetadata({ params: { locale } }: { params: { locale: Locale } }): Promise<Metadata> {
  const t = await getTranslations({ locale, namespace: 'services_telegram_bots' });
  return {
    title: t('meta_title'),
...
export default async function TelegramBotsPage({
  ...
  const benefitIcons = [
    ...
    <BarChart3 key="bar" size={24} className="text-laser-cyan" />,
    <Brain key="brain" size={24} className="text-laser-cyan" />
  ];

  const useCaseIcons = [
    <Headphones key="headphones" size={20} className="text-laser-cyan" />,
    <ShoppingCart key="cart" size={20} className="text-laser-cyan" />,
    <Bell key="bell" size={20} className="text-laser-cyan" />,
    <Users key="users" size={20} className="text-laser-cyan" />,
    <Webhook key="webhook" size={20} className="text-laser-cyan" />,
    <BarChart3 key="bar" size={20} className="text-laser-cyan" />
  ];
