...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  const t = await getTranslations({ locale: params.locale, namespace: 'services_research' });
  return {
    title: t('meta_title'),
...
export default async function ResearchPage({
  ...
  const t = await getTranslations({ locale, namespace: 'services_research' });

  const serviceIcons = [
    <Target key="target" size={24} className="text-laser-cyan" />,
    <PieChart key="piechart" size={24} className="text-laser-cyan" />,
    <FileSearch key="filesearch" size={24} className="text-laser-cyan" />,
    <MapPin key="mappin" size={24} className="text-laser-cyan" />,
    <Compass key="compass" size={24} className="text-laser-cyan" />,
  ...
  const deliverables = [0, 1, 2, 3, 4, 5].map((i) => ({
    title: t(`deliverables.${i}.title`),
    desc: t(`deliverables.${i}.description`)
  }));

  const comparison = {
    intuition: [0, 1, 2, 3, 4, 5].map((i) => t(`comparison_intuition.${i}`)),
    data: [0, 1, 2, 3, 4, 5].map((i) => t(`comparison_data.${i}`))
  };
  ...
  const useCases = [0, 1, 2, 3, 4, 5].map((i) => ({
    title: t(`use_cases.${i}.title`),
    desc: t(`use_cases.${i}.description`)
  }));
